...
public class SMSNotificationRepositoryImpl extends SearchInterfaceImpl implements SMSNotificationRepository<SMSNotification> {
	...
	public List<SMSNotification> save(List<SMSNotification> entity) throws Exception {
		...
		for (int i = 0; i < entity.size(); i++) {
			SMSNotification obj = entity.get(i);
	...
	public void delete(String id) throws Exception {
		javax.persistence.EntityManager emanager = emfResource.getResource();
		SMSNotification s = emanager.find(SMSNotification.class, id);
		emanager.remove(s);
