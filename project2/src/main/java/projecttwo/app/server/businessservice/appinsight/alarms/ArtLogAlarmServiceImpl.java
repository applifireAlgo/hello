...
import projecttwo.app.shared.appinsight.alarms.ArtLogArchitectureLayer;
...
import projecttwo.app.shared.appinsight.alarms.ArtLogConnector;

import projecttwo.app.shared.appinsight.alarms.ArtDomain;

import projecttwo.app.shared.appinsight.alarms.ArtLogEventAction;

import projecttwo.app.shared.appinsight.alarms.ArtLogException;

import projecttwo.app.shared.appinsight.alarms.ArtLogSeverity;

import projecttwo.app.shared.appinsight.alarms.ArtLogStatus;

import projecttwo.app.server.repository.appinsight.alarms.ArtAlarmLoggerRepository;

import projecttwo.app.server.repository.appinsight.alarms.ArtBoundedContextRepository;

import projecttwo.app.server.repository.appinsight.alarms.ArtDomainRepository;
...
public class ArtLogAlarmServiceImpl extends ArtLogAlarmService {
	...
	public HttpEntity<ResponseBean> getListOfAlarms() {
		...
		/*
		 * List<ArtLogBoundedContext> boundedContexts =
		 * artLogBoundedContextRepository.findAll(); try { for (int i = 0; i <
		 * boundedContexts.size(); i++) { JSONObject jsonObject = new
		 * JSONObject(); String bId =
		 * boundedContexts.get(i).getBoundedContextId(); jsonObject.put("bId",
		 * bId); jsonObject.put("contextName", new
		 * JSONObject(boundedContexts.get
		 * (i).getBoundedContextJson()).getString("contextName"));
		 * jsonObject.put("contextPrefix",
		 * boundedContexts.get(i).getBoundedContextPrefix()); List<ArtLogDomain>
		 * domainData = boundedContexts.get(i).getArtLogDomain(); if (domainData
		 * != null && !domainData.equals("")) { JSONArray domainArray = new
		 * JSONArray(); for (int j = 0; j < domainData.size(); j++) { JSONObject
		 * domainObject = new JSONObject(); domainObject.put("mappingId",
		 * domainData.get(j).getdomainPkId()); domainObject.put("domainPrefix",
		 * domainData.get(j).getDomainId()); domainObject.put("domain",
		 * domainData.get(j).getDomainName()); domainArray.add(domainObject); }
		 * jsonObject.put("domains", domainArray); }
		 * contextArray.add(jsonObject); } } catch (JSONException e) {
		 * e.printStackTrace(); responseBean.add("success", true);
		 * responseBean.add("message", "Error in alarms retrieval"); return new
		 * org.springframework.http.ResponseEntity<ResponseBean>(responseBean,
		 * httpStatus); }
		 */
	...
	public HttpEntity<ResponseBean> updateLogAlarm(/*
													 * @RequestBody ArtLogDomain
													 * logDomain
													 */) {
		ResponseBean responseBean = new ResponseBean();
		org.springframework.http.HttpStatus httpStatus = org.springframework.http.HttpStatus.OK;
		/*
		 * ArtLogDomain awsLogDomain = null; if (logDomain.getdomainPkId() !=
		 * null) { awsLogDomain =
		 * artLogDomainRepository.findById(logDomain.getdomainPkId());
		 * logDomain.
		 * setArtLogBoundedContext(awsLogDomain.getArtLogBoundedContext());
	...
	public HttpEntity<ResponseBean> getGridData(@RequestBody String domainId) throws SecurityException {
		...
		/*
		 * List<ArtLogBoundedContext> boundedContexts =
		 * artLogBoundedContextRepository.findAll(); for
		 * (Iterator<ArtLogBoundedContext> iterator =
		 * boundedContexts.iterator(); iterator.hasNext();) {
		 * ArtLogBoundedContext artLogBoundedContext = (ArtLogBoundedContext)
		 * iterator.next(); List<ArtLogDomain> domainData =
		 * artLogBoundedContext.getArtLogDomain(); for (Iterator<ArtLogDomain>
		 * iterator2 = domainData.iterator(); iterator2.hasNext();) {
		 * ArtLogDomain artLogDomain = (ArtLogDomain) iterator2.next(); if
		 * (artLogDomain != null &&
		 * artLogDomain.getdomainPkId().equals(domainId)) {
		 * 
		 * List<ArtLogAlarm> artLogAlarms = artLogDomain.getArtLogAlarm(); if
		 * (artLogAlarms.size() > 0) { responseBean.add("success", true);
		 * responseBean.add("message", "Successfully retrived ");
		 * responseBean.add("data", artLogDomain); return new
		 * org.springframework.http.ResponseEntity<ResponseBean>(responseBean,
		 * httpStatus); } } } }
		 */
		responseBean.add("success", true);
