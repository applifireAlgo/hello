Ext.define('Project2.project2.web.projecttwo.controller.appinsight.health.EmployeeMainController', {
     extend: 'Project2.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.EmployeeMainController',
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#EmployeeForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#EmployeeGrid');
               var tree = this.view.down('#EmployeeTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#EmployeeForm');
          var grid = this.view.down('#EmployeeGrid');
          var tree = this.view.down('#EmployeeTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#EmployeeGrid');
               ...
               var formPanel = this.getView().up().down('#EmployeeForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#EmployeeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#EmployeeForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridConfig: function(data) {
          ...
          var gridStore = Ext.create('Ext.data.Store', {
               model: 'Project2.project2.shared.projecttwo.model.appinsight.health.EmployeeModel',
               autoLoad: true,
