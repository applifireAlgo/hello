Ext.define('Project2.project2.web.projecttwo.controller.appinsight.health.BugMainController', {
     extend: 'Project2.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.BugMainController',
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#BugForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#BugGrid');
               var tree = this.view.down('#BugTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#BugForm');
          var grid = this.view.down('#BugGrid');
          var tree = this.view.down('#BugTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#BugGrid');
               ...
               var formPanel = this.getView().up().down('#BugForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#BugTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#BugForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridConfig: function(data) {
          ...
          var gridStore = Ext.create('Ext.data.Store', {
               model: 'Project2.project2.shared.projecttwo.model.appinsight.health.BugModel',
               autoLoad: true,
